...
def gene_csqs_case_builder(in_mt):
    ...
    ht0 = (
        mt.group_rows_by(
            mt.consequence.vep.worst_csq_by_gene_canonical.gene_id).aggregate(
            s0=hl.agg.any(
                mt.a0_alt)))
    ht1 = (
        mt.group_rows_by(
    ...
    ht2 = (
        in_mt.group_rows_by(
            in_mt.consequence.vep.worst_csq_by_gene_canonical.gene_id).aggregate(
            hom_var=hl.agg.any(
                in_mt.GT.is_hom_var())))
    # combine entries
...
def calc_p_ko(mt):
    ...
    ko_mt = mt.annotate_entries(
        pKO=hl.if_else(
            ...
            hl.if_else(
                mt.DS == 1,
                hl.if_else(mt.singletons >= 1, 1 - (1 / 2) **
                           mt.singletons, 0),  # one phased hetz
                hl.if_else(mt.singletons >= 2, 1 - 2 * (1 / 2) **
                           mt.singletons, 0),  # zero phased hetz
            )
        )
    )
    return ko_mt
...
def gene_csqs_calc_pKO(mt_phased, mt_unphased,
                       fields_drop=['DS', 'singletons']):
    ...
    mt_ko = mt1_burden.annotate_entries(
        singletons=mt2_burden[(mt1_burden.gene_id, mt1_burden.s)].n)
    mt_ko = mt_ko.annotate_entries(
        singletons=hl.if_else(
            ~hl.is_missing(
                mt_ko.singletons),
            mt_ko.singletons,
            0))
...
def gene_csqs_calc_pKO_pseudoSNP(mt1, mt2, chrom):
    ...
    # multiply probability by 2 (dosage encoded [0:2])
    pmt = pmt.annotate_entries(DS=pmt.pKO * 2)
    pmt = pmt.drop('pKO')
...
def gene_strand_builder(mt, field='snpid'):
    ...
    strand1 = mt.filter_entries((mt.a0_alt == True) | (
        mt.a_homo == True)).entries()  # sets entries to NA in matrix table
    strand2 = mt.filter_entries(
        (mt.a1_alt == True) | (
            mt.a_homo == True)).entries()

    # filter to each gene
    strand1 = strand1.group_by(
        strand1.gene_id,
        strand1.s).aggregate(
        phase1=hl.agg.collect(
            strand1.rsid_entry))
    strand2 = strand2.group_by(
        strand2.gene_id,
        strand2.s).aggregate(
        phase2=hl.agg.collect(
            strand2.rsid_entry))
