...
#SBATCH --job-name=spa_null_shuffle
#SBATCH --chdir=/well/lindgren-ukbb/projects/ukbb-11867/flassen/projects/KO/wes_ko_ukbb
#SBATCH --output=logs/spa_null_shuffle.log
#SBATCH --error=logs/spa_null_shuffle.errors.log
...
readonly pheno_dir="data/phenotypes/shuffle_cases"
...
fit_binary_traits() {
  ...
  local out_dir="data/saige/output/binary/shuffle/step1"
  local pheno_list="${header_dir}/dec22_phenotypes_binary_200k_header.tsv"
  local phenotype=$( sed "${index}q;d" ${pheno_list} )
  local out="${out_dir}/${out_prefix}_${phenotype}"
  pheno_file="${pheno_dir}/dec22_phenotypes_binary_200k_shuffled_seed1.txt.gz"
    ...
    if [ ! -z ${phenotype} ]; then
      if [ ! -f "${out_prefix}.rda" ]; then
        if [ "${chr_done}" -lt "22" ]; then
          ...
          local slurm_lname="logs/_spa_null_shuffle"
          local slurm_project="${project}"
          local slurm_queue="${queue}"
          local slurm_nslots="${nslots}"
          if [ ${cluster} == "slurm" ]; then
            readonly spa_null_shuffle_jid=$( sbatch \
