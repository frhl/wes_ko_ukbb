...
main <- function(args){
    ...
    # get upper triangle
    header_mat <- expand.grid(header, header)
    dups <- duplicated(apply(header_mat, 1, function(x) paste(sort(x), collapse="")))
    stopifnot(length(dups) == nrow(header_mat))
    header_mat <- header_mat[!dups,]
    header_mat <- header_mat[header_mat$Var1 != header_mat$Var2,]
