...
input_prefix <- "/well/lindgren-ukbb/projects/ukbb-11867/flassen/projects/KO/wes_ko_ukbb/data/survival/knockouts/pLoF_damaging_missense/wo_chrom_prefix/ukb_eur_wes_200k_pLoF_damaging_missense_"
out_prefix <- paste0("/well/lindgren-ukbb/projects/ukbb-11867/flassen/projects/KO/wes_ko_ukbb/data/survival/2401_analyses/results/ref_group_", 
                     args$refGroup, "/unconditioned/only_chets/", args$fileType, "_chunk", args$chunk)


#input_prefix <- paste0("/well/lindgren-ukbb/projects/ukbb-11867/flassen/projects/KO/wes_ko_ukbb_nexus/data/survival/samvida/2401/UKB.carrier_matrix.eur.unrel.af05.pp0.90.pLoF_damaging_missense.")
...
id_class <- lapply(GENES, function (g) {
  res <- read.table(paste0(input_prefix, g, ".txt.gz"), 
...
#  write("WARNING: We are subsetting to homozygotes only!", stderr()) 
#  if (args$refGroup == "het") {
#    res <- res %>% mutate(group = ifelse(group == "hom", "knockout", group))
#  }
 
  write("WARNING: We are subsetting to CHs only!", stderr()) 
  if (args$refGroup == "het") {
    res <- res %>% mutate(group = ifelse(group == "chet_trans", "knockout", group))
  }
