...
| `03_make_genes.sh` | For each gene, generate a file containing the binominal probability of a sample being a knockout.   | `<02_count_hets.sh>`  |
| `04_permute.sh` | Master script stratified by chromosome. Will start a new script (`_permute.sh`) for each gene on the current chromosome, that will shuffle phases based on the underlying binominal distribution specified in (2-3). The script will then perform the following operations: </br>1. Check if any permutations are required based, and if so it will submit an array job (`_gene_permute.sh` calling `_gene_permute.R`) with number of tasks equal to number of permutations currently required. If not, the script will exit and no more iterations will be performed. </br>2. Submit a array job (`_gene_spa.sh`) performing saddlepoint approximation with SAIGE for each phenotype that still requires permutations. </br>3. Submit a job (`_merge_spa.sh`) that merges the saddlepoint approximated phenotypes (one merged file for each phenotype). </br>4. Submit a job (`_calc_p.sh` calling `_get_spa_p.R`, `_logic.R`,`_calc_empirical_p.R`) that calculates the current empirical P-value by comparing the true P-values and the shuffled P-values originating from the permuted phase. </br>5. Submit itself again but (`_permute.sh`) increasing the number of shuffles required by a factor fo 10 for phenotypes that are not yet appropiately permuted. |`03_make_genes.sh`, `02_count_hets.sh` and ``01_calc_min_permutations.sh``  |
| `05_collect.sh` | Aggregates the permuted output in a manner that can easily be read by other software.  | `04_permute.sh`  |
