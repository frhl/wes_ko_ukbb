...
| `03_wes_naive_phasing.sh` | 1. Open whole exome sequencing data </br> 2. Run SHAPEIT4 phasing with [default parameters](https://odelaneau.github.io/shapeit4/) using the `--sequencing` flag. </br> 3. Index result VCF and create `.tbi` files. </br> 4. Calculate combined (trio) switch errors using BCFtools. | N/A |
| `03_calls_naive_phasing.sh` | 1. Open genotype calls </br> 2. Run SHAPEIT4 phasing using default parameters. </br> 3. Index result VCF and create `.tbi` files. </br> 4. Calculate combined (trio) switch errors using BCFtools. | `01_calls_gen.sh` |
