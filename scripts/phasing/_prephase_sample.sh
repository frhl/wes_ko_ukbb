...
readonly path_unphased_gz="${out_prefix}/s${sample_idx}_eid${eid}.vcf.gz"
readonly path_phased="${out_prefix}/s${sample_idx}_eid${eid}_phased.vcf.gz"
...
# make tabix
if [ ! -f "${path_phased}.tbi" ]; then
  make_tabix "${path_phased}" "tbi"
fi
