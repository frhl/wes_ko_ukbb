...
def main(args):
    ...
    mt = mt.annotate_rows(worst_csq_for_variant_canonical=vep[mt.row_key].vep.worst_csq_for_variant_canonical)

    # annotate with category
    annotations = analysis.annotation_case_builder(
        mt.worst_csq_for_variant_canonical, use_loftee=True)
    mt = mt.annotate_rows(consequence_category=annotations)

    # count by samples
    mt_samples = analysis.count_urv_by_samples(mt)
    mt_samples.entries().flatten().export(out_prefix + "_variants_by_samples.tsv.bgz")
