...
Filter to genes with a significant compound_het variant association, move to step 2.
Filter to genes with significant compound_het variant association after also conditioning on LOCO PRS, move to step 3.
Filter to genes with significant compound_het variant association after additionally conditioning on all common variant signals in the region using an iterative procedure. Are there any left?
We now have a couple of options
What Wei called 'reciprocal conditioning'. Run standard SAIGE gene on rare variants in these genes conditioning on off chr PRS, common signal nearby - get a set of P-values. Run the same test, but, additionally, condition on the compound het signal - did the signal disappear for any of the tests? If it did, we have a candidate association that's driven by compound hets.
Just run the brute force approach of conditioning out all rare variant PTVs in the gene, and see if there's still a significant association between presence/absence of the compound het.
