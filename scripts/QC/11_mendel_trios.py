...
def main(args):
    ...
    mt = mt.annotate_rows(consequence_category = annotation) 

    # write out results
    mt_result_all = (mt.group_rows_by(mt.consequence_category).
    ...
    mt_result_all.entries().flatten().export(out_prefix + "_mendel_all.tsv.bgz")

    # write out only for singletons
    mt = mt.filter_rows(mt.info.AC[1] == 1)
    mt_result_singletons = (mt.group_rows_by(mt.consequence_category).
            aggregate(variant = hl.struct(
