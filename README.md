...
| `06_create_grm.sh` | 1. Load all autosomes from ukbiobank genotype calls and subset to variants in `/well/lindgren/UKBIOBANK/DATA/QC/ukb_snp_qc.txt` with `in_Relatedness==1`.  </br> 2. Perform subsets based Duncan's [QCed samples](https://github.com/astheeggeggs/SAIGE_gene_munging/tree/main/QC_scripts). </br> 3. Write plink files. </br>  4. use SAIGE to fit sparse genetic relatedness matrix  | N/A |
