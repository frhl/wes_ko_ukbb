...
| `02_qc_input_mt.sh` | 1. Perform subsets based Duncan's [QCed samples and variants](https://github.com/astheeggeggs/SAIGE_gene_munging/tree/main/QC_scripts). </br> 2. Remove samples that have withdrawn consent. </br> 3. Annotate with sites found in gnomAD. </br> 4. Annotate with sites found in imputed data. </br> 5. Export variant and sample level statistics using `hl.sample_qc` and `hl.variant_qc` as .tsv files. </br> 6. Annotate and export VEP consequence counts by gene. | `01_hail_vep.sh` |
